...
// ICW1 (we don't need to set A0 just tells us if we need to write to the
// command or data segment, A0 = 1 -> data, A0 = 0 -> Code)
...
uint8_t M_OFFSET = 0b100; // this is an 8 bit number, where the bit selected =
                          // id (e.g. 0 = id 0, 10 = 1, 100 = 2, 1000 = 3, etc)
...
// I am going to presume for now that we will not define the OCW flags and will
// just setup PIC but if not the definitions are page 13 of intel manual for PIC

// at some point could use for setting/getting interrupt masks
void OCW(uint8_t flags) {}

// we will just use predefined values but theoretically we could make functions
// for this
void initialize_pic() {
  asm volatile("cli");
  ...
  io_wait();
  outb(PIC_M_DATA,
       (uint8_t)0x20); // after ISR 31 the rest are free so just start here
  io_wait();
